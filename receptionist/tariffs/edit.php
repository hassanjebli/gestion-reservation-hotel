...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql = $conn->prepare('SELECT * FROM tarif_chambre WHERE id_tarif=?');
    $sql->execute([$id]);
    ...
    if (isset($_POST['modifier'])) {
        ...
        if (!empty($prix_base_nuit) && !empty($prix_base_passage) && !empty($n_prix_nuit) && !empty($n_prix_passage)) {
            $sqlState = $conn->prepare('UPDATE tarif_chambre SET prix_base_nuit=?,prix_base_passage=?,n_prix_nuit=?,n_prix_passage=? WHERE id_tarif =?');
            $sqlState->execute([$prix_base_nuit, $prix_base_passage, $n_prix_nuit, $n_prix_passage,$id]);
            $_SESSION['message'] = 'La modifaction a ete effectue!!';
            ...
            <div class="col-10 mx-auto">
                <div class="row">
                    <div class="form-group col-6">
                        ...
                        <input type="number" name="prix_base_nuit" class="form-control" step="any" value="<?=$tarif['prix_base_nuit'] ??""?>">
                    </div>
                    <div class="form-group col-6">
                        <label class="form-label">prix base passage </label>
                        <input type="number" name="prix_base_passage" class="form-control" step="any" value="<?=$tarif['prix_base_passage'] ??""?>">
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-6">
                        <label class="form-label">n prix nuit</label>
                        <input type="number" name="n_prix_nuit" class="form-control" step="any" value="<?=$tarif['n_prix_nuit'] ??""?>">
                    </div>
                    <div class="form-group col-6">
                        <label class="form-label">n prix passage</label>
                        <input type="number" name="n_prix_passage" class="form-control" step="any" value="<?=$tarif['n_prix_passage'] ??""?>">
                    </div>
                </div>
                <div class="row my-3">
